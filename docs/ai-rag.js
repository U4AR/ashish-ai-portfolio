(()=>{
  ...
  enable.addEventListener('click',()=>{
    ...
    worker=new Worker('ai-worker.js',{type:'module'});worker.onmessage=({data})=>{
      ...
      if(data.type==='error'){status.textContent=`Local model could not load: ${data.message}. Retrieval remains available.`;progress.hidden=true;enable.disabled=false;enable.textContent='Retry Gemma 4 E2B QAT download';generate.disabled=true;worker?.terminate();worker=null}
    };worker.onerror=event=>{status.textContent=`Local model worker failed: ${event.message||'unknown error'}. Retrieval remains available.`;enable.disabled=false;enable.textContent='Retry Gemma 4 E2B QAT download';progress.hidden=true;worker=null};worker.postMessage({type:'init'});
  });
