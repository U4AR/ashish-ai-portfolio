(()=>{
  const byId=id=>document.getElementById(id), form=byId('ragForm'), query=byId('ragQuery'), output=byId('ragOutput'), evidence=byId('ragEvidence'), answer=byId('ragAnswer'), enable=byId('enableAI'), choice=byId('localModelChoice'), generate=byId('generateAnswer'), status=byId('modelStatus'), progress=byId('modelProgress');
  ...
  enable.addEventListener('click',()=>{
    ...
    const selected=choice.value, details=selected==='large'?{label:'Gemma 4 E2B q4f16',size:'3.38 GB'}:{label:'Gemma 3 1B q4f16',size:'0.8 GB'};
    const approved=confirm(`This will download about ${details.size} for ${details.label} from Hugging Face and allocate GPU memory. Nothing has been downloaded yet. Continue?`);
    if(!approved)return;
    enable.disabled=true;choice.disabled=true;enable.textContent='Loading Gemma…';progress.hidden=false;status.textContent='Starting local model worker…';
    worker=new Worker('ai-worker.js?v=20260712-6',{type:'module'});worker.onmessage=({data})=>{
      ...
      if(data.type==='ready'){modelReady=true;progress.value=100;progress.hidden=true;enable.textContent=`${data.label||details.label} enabled`;status.textContent='Local model ready. Model data remains in this browser session/cache.';generate.disabled=!retrieved.length}
      ...
      if(data.type==='error'){status.textContent=`Local model could not load: ${data.message}. Retrieval remains available.`;progress.hidden=true;enable.disabled=false;choice.disabled=false;enable.textContent='Retry selected model download';generate.disabled=true;worker?.terminate();worker=null}
    };worker.onerror=event=>{status.textContent=`Local model worker failed: ${event.message||'unknown error'}. Retrieval remains available.`;enable.disabled=false;choice.disabled=false;enable.textContent='Retry selected model download';progress.hidden=true;worker=null};worker.postMessage({type:'init',model:selected});
  });
